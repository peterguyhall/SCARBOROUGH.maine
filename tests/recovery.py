...
def _poll_endorsements(node, tx, *, expect_status, attempts=60, sleep_s=0.5):
    ...
    than the partial-Done bootstrap grace (5 attempts of exponential
...
def run_recovery_endorsement_chain_heals(args):
    ...
    S1-era receipts get HTTP 202 (the subsystem keeps polling at the
    configured 1s cap). Once the chunk is restored, the periodic poll heals
    the chain and the S1-era receipts succeed.
    """
    txs = app.LoggingTxs("user0")
